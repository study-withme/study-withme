...
### 합격시그널 – 프로그래머스 데브코스 백엔드 3차 팀 프로젝트
- **프로젝트 유형**: 프로그래머스 데브코스 백엔드 엔지니어링 3차 프로젝트 (6인 팀)
- **프로젝트 개요**: 2차 프로젝트로 개발한 취준생 커뮤니티 백엔드의 Java 코드를 Kotlin으로 마이그레이션
- **담당 역할**: 댓글·대댓글 조회/삭제 및 댓글 좋아요 기능의 Kotlin 전환
- **핵심 구현**: 기존 Java 기반 댓글 도메인의 2Depth 조회·삭제 로직과 권한·존재성 검증 로직을 Kotlin으로 재구현
- **기술 포인트**: Kotlin의 null-safety와 간결한 문법을 적용하고, 좋아요 insert/delete 토글 방식과 기존 데이터 정합성 유지
- **마이그레이션 성과:** 기존 API 명세와 동작을 유지하면서 Java 코드를 Kotlin으로 전환하고 테스트를 통해 기능 회귀 검증
- **협업/운영:** 마이그레이션 과정에서 코드 변환 기준을 공유하고 PR 리뷰·충돌 조율·머지 흐름 관리
- **주요 기술**: Kotlin, Spring Boot, Spring Data JPA, Spring Security, Redis, Next.js
- **Repo 바로가기**: [NBE9-11-3-Team01](https://github.com/study-withme/NBE9-11-3-Team01)
